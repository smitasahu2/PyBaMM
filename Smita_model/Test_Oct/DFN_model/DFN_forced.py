...
"""""
test1 = 1  # pa2346/60ram["Current function [A]"] = 5/4, C/4 c-rate
test2 = 2  # param["Current function [A]"] = 5/2, C/2 c-rate
test3 = 3  # param["Current function28.5 [A]"] = 5, pybamm's default value 1C rate
...
for test in testall:
    if test == 1:
        ...
        ["Discharge at 0.25C for 14400 seconds",
        "Charge at 0.25C for 13770 seconds",
        ]#*5  # final capacity check
         )      
        # params["Current function [A]"] = 5/4
        print("Current function 5/4 [A]: C/4 c-rate")
        #t_eval = np.linspace(0, 28170, 10000)
    ...
    if test == 4:
        ...
        standard_protocol = pybamm.Experiment(
        ["Discharge at 1.5C for 2290 seconds",
        "Charge at 1.5C for 1268 seconds"
        ]#*5  # final capacity check
         )      
        sim_DFN = pybamm.Simulation(model_DFN, parameter_values=params, var_pts=var_pts, experiment=standard_protocol)
        sol_DFN = sim_DFN.solve()
    ...
    if test == 5:
        ...
        standard_protocol = pybamm.Experiment(
        ["Discharge at 2C for 1623 seconds",
        "Charge at 2C for 723 seconds",
        ]#*5  # final capacity check
         )
        t_eval = np.linspace(0, 2346, 10000)
        sim_DFN = pybamm.Simulation(model_DFN, parameter_values=params, var_pts=var_pts, experiment=standard_protocol)
