...
def run_SEI_test(all_test1, kpe):
    ...
    savetxt(f'plate_kpe{kpe_str}.csv', C_Li, delimiter=',')

    Dead = sol_all["Dead lithium concentration [mol.m-3]"].entries
    savetxt(f'plate_kpe{kpe_str}.csv', Dead, delimiter=',')

    sol_all.save_data(f"SEI_D{kpe_str}.csv",
                      [
                        "Time [min]", 
                        "Current [A]", 
                        "Terminal voltage [V]", 
...
for i, all_test1 in enumerate(all_test1_values):
    run_SEI_test(all_test1, kpe_values[i])
