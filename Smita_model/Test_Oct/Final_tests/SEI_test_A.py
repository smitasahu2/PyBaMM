...
def run_SEI_test(SEI_test1, diffusivity):
    ...
    # Create the subfolder based on the diffusivity_str variable
    ...
    try:
      ...
      #print(f"Subfolder name: {diffusivity_str}")
      ...
      #DandeLiion_current = pd.read_csv("total_current.csv", comment="#", header=None).to_numpy() 
    # create interpolant
    ...
    # set drive cycle
      ...
      # Save Negative electrode porosity to a CSV file
      porosity = sol_SEI["Negative electrode porosity"].entries
      savetxt(os.path.join(output_folder, f"porosity_D{diffusivity_str}.csv"), porosity, delimiter=',')
...
SEI_test1_values = [1]#[1, 2, 3, 4]  # Add all the test cases we want to run
diffusivity_values = [1.25e-20]#[2.5e-22, 2.5e-21, 7.5e-21, 1.25e-20]  # Corresponding diffusivity values
