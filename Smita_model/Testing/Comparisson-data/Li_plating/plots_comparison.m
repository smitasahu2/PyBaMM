...
figure('Name', 'Li plating')

subplot(1,2,1)
time = [0, 100,200,400,800,1000,1200,1400,1600,2000, 2100,2200,2300,2400,2500,3600,3660,4000,4100, 4200,4400,4800,4900,5000,5100,5200,5300,5400,5500,5600,5700,5800,5900,6000]/3600;%,6090];
...
for i =[2.5e-22]%, 2.5e-21,7.5e-21, 1.25e-20]
    SEI =    importdata(['on_SEI_D' num2str(i) '.csv']) ;
    C_Li =    importdata(['C_Li_D' num2str(i) '.csv']) ;
    plot(SEI.data(:,1)/60, C_Li(1,:),'-x','LineWidth',1.5,'MarkerSize',1,'DisplayName', ['i=' num2str(i)]);
    clear
end
xlabel('Time [hrs]','Interpreter','latex','fontsize',12);
ylabel('Plated Li concentration [molm$^-3$]','Interpreter','latex','fontsize',12);
legend('DandeLiion', 'pybamm','Interpreter','latex','fontsize',10,'location','best');
...
for i =[2.5e-22]%, 2.5e-21,7.5e-21, 1.25e-20]
    SEI =    importdata(['on_SEI_D' num2str(i) '.csv']) ;
    Dead =    importdata(['Dead_D' num2str(i) '.csv']) ;
    plot(SEI.data(:,1)/60, Dead(1,:),'-x','LineWidth',1.5,'MarkerSize',1,'DisplayName', ['i=' num2str(i)]);
    clear
end
xlabel('Time [hrs]','Interpreter','latex','fontsize',12);
ylabel('Dead Li concentration [molm$^-3$]','Interpreter','latex','fontsize',12);
legend('DandeLiion', 'pybamm','Interpreter','latex','fontsize',10,'location','best');
