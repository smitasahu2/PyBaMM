...
def LFP_ocp(sto):
    ...
    T : :class:`pybamm.Symbol`
         Temperature [K]
    c_s_rav : :class:`pybamm.Symbol`
        Average concentration in LFP cathode particle [mol.m-3]
    c_s_max : :class:`pybamm.Symbol`
        Maximum particle concentration [mol.m-3]
    mu_Li   : electrochemical potential in solid [V]
    mu_Li_plus  : Reference electrochemical potential (constant) in electrolyte [V]
    mu_e_0      : Reference electrochemical potential (constant) in electrolyte after de-intercalation [V]
    ...
    F = 96485.33212
    # equation 13
    mu_Li = R * T * (np.log(sto / (1 - sto)) + om * (1 - 2 * sto))
    mu_Li_plus = -328873.86163228
    mu_e_0 = 0
    phi_eq = 1 / F * (mu_Li - mu_Li_plus - mu_e_0)  # equation 12
    return phi_eq
...
def LFP_exch(c_e, c_s_surf, c_s_max, T):
    ...
    k   : Fitting parameter exchange current density parameters [s-3]
    c_e : :class:`pybamm.Symbol`
         Electrolyte concentration [mol.m-3]
    T   : :class:`pybamm.Symbol`
         Temperature [K]
    ...
    # return k * np.exp(mu_Li / R / T / 2) * c_e**0.5
    return k * (1 - c_s_surf / c_s_max) ** 0.5 * (c_s_surf / c_s_max) ** 0.5
...
def get_parameter_values():
    ...
    return {
        ...
        "Positive electrode exchange-current density [A.m-2]" "": LFP_exch,
        ...
        "Lower voltage cut-off [V]": 3.37,
        "Upper voltage cut-off [V]": 3.43,
...
#     )
#
#     return k * pybamm.exp(mu_Li / pybamm.constants.R / T / 2) * c_e**0.5
#
#
