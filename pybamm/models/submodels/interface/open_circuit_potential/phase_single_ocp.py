...
class PhaseSingleOpenCircuitPotential(BaseOpenCircuitPotential):
        ...
        if self.reaction == "lithium-ion main":
            ...
            if domain_options["particle size"] == "distribution":
                ...
                if isinstance(sto_surf, pybamm.Broadcast) and isinstance(
                    ...
                    sto_surf = sto_surf.orphans[0]
                    T = T.orphans[0]
            mu = 1 * sto_surf  # mu(c) here
            ocp_surf = 2 * mu  # V = f(mu)
            # ocp_surf = self.phase_param.U(sto_surf, T)
            dUdT = self.phase_param.dUdT(sto_surf)  # a func of mu
